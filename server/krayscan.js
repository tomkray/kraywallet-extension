...
async function loadInscriptionDetails(txid, vout) {
    try {
        ...
        // Extrair inscription ID
        const inscriptionMatch = html.match(/<a href=\/inscription\/([a-f0-9]{64}i\d+)>/);
        if (!inscriptionMatch) {
            return '<div style="color: var(--color-text-secondary); font-size: 12px;">No inscription found</div>';
        }
        ...
        const numberMatch = inscHtml.match(/<h1>Inscription (\d+)<\/h1>/);
        if (numberMatch) {
        ...
        const contentTypeMatch = inscHtml.match(/<dt>content type<\/dt>\s*<dd>([^<]+)<\/dd>/i);
        if (contentTypeMatch) {

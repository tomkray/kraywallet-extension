...
class UTXOFilter {
    ...
    /**
     * Obter dados completos das runes em um UTXO
     * @param {string} txid - Transaction ID
     * @param {number} vout - Output index
     * @returns {Promise<Array>} Array com dados das runes [{name, amount, symbol}, ...]
     */
    async getRunesData(txid, vout) {
        try {
            const outpoint = `${txid}:${vout}`;
            const outputData = await quicknode.getOutput(outpoint);
            ...
            // outputData.runes formato: { "runeId": "amount" }
            const runesArray = [];
            
            for (const [runeId, amount] of Object.entries(outputData.runes)) {
                // Buscar nome da rune via QuickNode
                try {
                    const runeDetails = await quicknode.call('ord_getRune', [runeId]);
                    
                    runesArray.push({
                        name: runeDetails.spaced_rune || runeDetails.name || runeId,
                        amount: amount,
                        symbol: runeDetails.symbol || '⧈',
                        runeId: runeId,
                        divisibility: runeDetails.divisibility || 0
                    });
                    
                    console.log(`   🪙 Rune: ${runeDetails.spaced_rune || runeId} (${amount})`);
                    
                    await new Promise(r => setTimeout(r, 120));
                } catch (e) {
                    // Se não conseguir buscar detalhes, usar ID
                    runesArray.push({
                        name: runeId,
                        amount: amount,
                        symbol: '⧈',
            ...
            console.log(`   ℹ️  Could not get runes data for ${txid}:${vout}:`, error.message);
            return [];
        }
    }
